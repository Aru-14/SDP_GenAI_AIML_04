...
def cook_food_local(prompt):
    if prompt:
        # User feedback with toasts
        st.toast("Gathering local ingredients...")
        time.sleep(1)
        ...
        st.write("Let's see what the local model says:")
        
        try:
            # 1. Call your local model (ensure 'gemma2:2b' or 'gemma2' is pulled)
            response = ollama.chat(
                model='gemma2:2b', 
                messages=[{'role': 'user', 'content': f"Write a summarized recipe for: {prompt}"}]
            )
...
if __name__ == "__main__":
    st.title("🍳 Local Recipe Generator (Gemma 2)")
    
    # Check if Ollama is running
    with st.sidebar:
        st.info("Using Local Gemma 2 via Ollama")
        if st.button("Check Local Models"):
            models = ollama.list()
            st.write(models)
